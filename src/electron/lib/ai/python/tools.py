...
class addNodeTool(BaseTool):
    ...
    def _run(
        ...
        data = {"function": "addNode", "args": {"signature": signature}}
        api = get_api()
        api.send(data)
        res = api.receive()
        with open("log.txt", "w") as f:
            f.write(f"[{res}]")
            # f.write(json.dumps(json.loads(res), indent=4))
        return res
...
class removeNodeTool(BaseTool):
    ...
    def _run(
        ...
        data = {"function": "removeNode", "args": {"id": id}}
        api = get_api()
        api.send(data)
        res = api.receive()
        return res
...
class addEdgeTool(BaseTool):
    ...
    def _run(
        ...
        data = {"function": "addEdge", "args": {"output": output, "input": input}}
        api = get_api()
        api.send(data)
        res = api.receive()
        return res
...
class removeEdgeTool(BaseTool):
    ...
    def _run(
        ...
        data = {"function": "removeEdge", "args": {"id": id}}
        api = get_api()
        api.send(data)
        res = api.receive()
        return res
